...
exports.getSingleProductBySlug = async (req, res, next) => {
	...
	try {
		...
		if (data.campaignStartDate && data.campaignEndDate) {
			const startDateTime = utils.convertToDateTime(
				data.campaignStartDate,
				data.campaignStartTime
			);
			const endDateTime = utils.convertToDateTime(
...
exports.productFilterByQuery = async (req, res, next) => {
	try {
		...
		if (req.body.select) {
			queryProduct = Product.find({ $and: query }).select(req.body.select);
			// .populate('attributes')
			// .populate('tags')
...
exports.mkAdsTracUserId = async (req, res, next) => {
	try {
		const bodyData = req.body;
		const resData = await axios.post(
			"https://api.mkadsdigital.com/userativity/track",
			bodyData
		);
